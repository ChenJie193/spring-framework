...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				/**
				 * 本方法会实例化和调用（执行 invoke）所有BeanFactoryPostProcessor（包括其子类 BeanDefinitionRegistryPostProcessor）
				 * BeanFactoryPostProcessor -- 扩展的特殊bean可以通过实现此接口
				 */
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				/**
				 * 实例化并且注册BeanPostProcessor 此时不执行 后期执行 也属于bean之前的准备工作
				 */
				...
				// Instantiate all remaining (non-lazy-init) singletons. 实例化所有的剩下的非懒加载的，单例对象
				/**
				 * TODO 重要：完成bean的实例化
				 */
	...
	protected void prepareRefresh() {
		...
		//判断刷新前的应用程序监听器集合是否为空。如果为空。则监听器添加到此集合中
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//设置类加载器 设置BeanFactory的classloader为当前context的classloader
		...
		//设置忽略的aware接口，很多同学理解不了为什么此处要对这些接口进行忽略，原因非常简单，这些接口的实现是由容器通过set方法进行注入的
		...
		// 设置几个自动装配的特殊规则，当在进行ioc初始化的时候，如果有多个实现，那么使用指定的对象进行注入
		...
		// 添加对AspectJ的支持，在java中织入分为三种方式，分为编译器织入，类加载器织入，运行期织入，编译器织入是指在java编译期，采用特殊的编译器，将切面织入到java类中，
		// 而类加载期织入则指通过特殊的类加载器，在类字节码加载到JVM时，织入切面，运行期织入则是采用cglib和jdk进行切面的织入
		// aspectj提供了两种织入方式，第一种是通过特殊编译器，在编译期，将aspectj语言编写的切面织入到java类中，第二种是类加载期织入，就是下面的load time weaving
